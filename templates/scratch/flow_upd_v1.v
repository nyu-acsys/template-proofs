...
Section insert_flow_upd.
  ...
  Function insert_flow_updk_rec (es: gmap Node esT)
  ...
  | true =>
    ...
    | Some n1 => 
      ...
      let In := I' !!! n in
      (* Add k to outf(In, n1) *)
      let In' := outflow_delete_set In n1 {[k]} in
      (* Pick from I because n1 must be untouched so far *)
      let In1 := I !!! n1 in
      (* Add k to inf(In1, n1) *)
      let In1' := inflow_delete_set In1 n1 {[k]} in
      let II := <[n := In']> I' in
      let II' := <[n1 := In1']> II in  
      insert_flow_updk_rec es I k n1 (R ∖ {[n]}) II' end end.
  ...
  Definition insert_flow_updk_post (I II: gmap Node (multiset_flowint_ur K)) :=
    let f := λ m1 m2,
              match m1, m2 with 
  ...
  Definition insert_flow_updk es I k n0 :=
    match (es !!! n0) !! k with
    | None => None
    | Some n1 =>
      let In0 := I !!! n0 in
      let In0' := outflow_delete_set In0 n1 {[k]} in
      let In1 := I !!! n1 in
      let In1' := inflow_delete_set In1 n1 {[k]} in
      let I' := <[n1 := In1']> ({[n0 := In0']}) in
      match insert_flow_updk_rec es I k n1 (dom I ∖ {[n0]}) I' with
      | Some (II, nk) =>
        let II' := insert_flow_updk_post I II in
        Some (II', nk)
      | None => None end end.

  Lemma insert_flow_updk_rec_defined rank es I k n R I':
    (es_rank_rel es rank) →
    (es_closed es I) →
    (n ∈ dom I') →
    (dom I = R ∪ dom I') →
    ...
    (∀ x, x ∈ dom I' → (rank x ≤ rank n)%Qc) →
      insert_flow_updk_rec es I k n R I' ≠ None.
  Proof.
    apply insert_flow_updk_rec_ind; try done.
