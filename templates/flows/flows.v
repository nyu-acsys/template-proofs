    ...
    out I1' n = out I1 n + m →
      inf I2' n = inf I2 n + m →
        (∀ n', n' ≠ n → out I1' n' = out I1 n') →
          (∀ n', n' ≠ n → inf I2' n' = inf I2 n') →
            (∀ n', inf I1' n' = inf I1 n') →
              (∀ n', out I2' n' = out I2 n') →
                domm I1' = domm I1 → domm I2' = domm I2 →
                  I1' ⋅ I2' = I1 ⋅ I2.
Proof.
  intros Valid Domm_I1 n_in_I2 Out1_n Inf2_n Out1_n' Inf2_n' Inf1 Out2 Domm1 Domm2.
  assert (✓ (I1' ⋅ I2')) as Valid'.
  { apply intValid_composable.
    repeat split.
    - apply intValid_unfold. repeat split.
      + intros H'. rewrite H' in Domm1.
        unfold domm at 1 in Domm1.
        unfold dom, flowint_dom, inf_map in Domm1.
        rewrite dom_empty_L in Domm1.
        rewrite Domm1 in Domm_I1. done.
      + apply map_disjoint_dom.
        destruct I1' as [ I1' | ]; last first.
        * admit.
        * destruct I1' as [inf1' out1'].
          unfold inf_map, out_map. simpl.
          destruct I1 as [I1 | ]; last first.
          { admit. }
          destruct I1 as [inf1 out1].
          assert (out1' ! n = 0 → 
          ...
          { intros H'.
            ...
            { intros n'. destruct (decide (n' = n)).
              - subst n'. rewrite nzmap_elem_of_dom_total.
                intros H''. clear -H' H''. try done.
              - rewrite nzmap_elem_of_dom_total.
                intros H''. 
                assert (n' ∈ dom (nzmap_car out1) → 
                          n' ∈ dom (nzmap_car out1) ∖ {[n]}) as H'''.
                { clear -n0. set_solver. }
                apply H'''; clear H'''. rewrite nzmap_elem_of_dom_total.
                pose proof Out1_n' n' n0 as H'''.
                unfold out, out_map in H'''. simpl in H'''.
                by rewrite H''' in H''. }         
            assert (dom (nzmap_car out1) ∖ {[n]} ⊆ dom (nzmap_car out1')) as H'''.
            { intros n'. destruct (decide (n' = n)).
              - subst n'. clear; intros ?; set_solver.
              - intros Hn'.
                assert (n' ∈ dom (nzmap_car out1)) as Hn''.
                { clear -n0 Hn'. set_solver. }
                rewrite nzmap_elem_of_dom_total.
                rewrite nzmap_elem_of_dom_total in Hn''.
                pose proof Out1_n' n' n0 as Out.
          ...
          { intros H'.
            assert (dom (nzmap_car out1') ⊆ dom (nzmap_car out1) ∪ {[n]}) as H''.
            { intros n'. destruct (decide (n' = n)).
              - subst n'. intros _; clear; set_solver.
              - rewrite nzmap_elem_of_dom_total.
                intros H''. 
                assert (n' ∈ dom (nzmap_car out1)) as H'''.
                { rewrite nzmap_elem_of_dom_total.
                  pose proof Out1_n' n' n0 as H'''.
                  unfold out, out_map in H'''. simpl in H'''.
                  by rewrite H''' in H''. }
            ...
            assert (dom (nzmap_car out1) ∪ {[n]} ⊆ dom (nzmap_car out1')) as H'''.
            { intros n'. destruct (decide (n' = n)).
              - subst n'. intros _; by rewrite nzmap_elem_of_dom_total. 
              - intros Hn'.
                assert (n' ∈ dom (nzmap_car out1)) as Hn''.
                { clear -n0 Hn'. set_solver. }
                rewrite nzmap_elem_of_dom_total.
          ...
          { destruct (decide (out1' ! n = 0)). 
            ...
            + right; by apply Hor2. }
          admit.   
      + intros H'. rewrite <-Domm1 in Domm_I1.
        unfold domm, dom, flowint_dom in Domm_I1.
        rewrite H' in Domm_I1.
        rewrite dom_empty_L in Domm_I1.
        exfalso; try done.
    - apply intValid_unfold. repeat split.
      + intros H'. rewrite H' in Domm2.
        unfold domm at 1 in Domm2.
        unfold dom, flowint_dom, inf_map in Domm2.
        rewrite dom_empty_L in Domm2.
        rewrite <-Domm2 in n_in_I2. 
        clear -n_in_I2; set_solver.
      + admit.
      + admit.
    - apply intComp_dom_disjoint in Valid.
      rewrite <-Domm1 in Valid. 
      by rewrite <-Domm2 in Valid.
    - apply map_Forall_lookup. intros n' m'.
      rewrite Inf1 Out2.
      admit.
    - apply map_Forall_lookup. intros n' m' Hn'.
      destruct (decide (n' = n)) as [? | H'].
      + subst n'.
        rewrite Out1_n Inf2_n.
        rewrite ccm_misc.
        admit.
      + rewrite (Out1_n' n' H').
        rewrite (Inf2_n' n' H').
  ...
  - rewrite !intComp_dom; try done.
    by rewrite Domm1 Domm2.
  - rewrite intComp_dom; try done.
    rewrite Domm1 Domm2.
    clear -n_in_I2. set_solver.
  - intros n'. 
    destruct (decide (n' ∈ domm (I1' ⋅ I2'))) as [H' | H'].
    + assert (n' ∈ domm (I1 ⋅ I2)) as H''.
      { admit. }
      destruct (decide (n' ∈ domm I1')) as [Hn' | Hn'].
      * assert (n' ∈ domm I1) as Hn''.
        { by rewrite <-Domm1. }
        Search inf.
        rewrite !intComp_inf_1; try done.
        by rewrite Inf1 Out2.
      * assert (n' ∈ domm I2') as Hn2'.
        { admit. }
        assert (n' ∈ domm I2) as Hn2''.
        { admit. }
        rewrite !intComp_inf_2; try done.
        destruct (decide (n' = n)) as [? | H'''].
        ** subst n'. rewrite Out1_n Inf2_n.
           by rewrite ccm_misc.
        ** rewrite (Out1_n' n' H''').
           by rewrite (Inf2_n' n' H''').
    + assert (n' ∉ domm (I1 ⋅ I2)) as H''.
      { admit. }
