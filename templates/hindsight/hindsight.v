...
Section Proof.
  ...
  Parameter Op : Type.
  Parameter snapshot : Type.
  Parameter snapshotUR : ucmra.
  Parameter test : snapshot -> snapshotUR.
  Coercion test : snapshot >-> snapshotUR.
  ...
  Parameter seq_spec_dec : ∀ op s s' res, Decision (seq_spec op s s' res).
  ...
  Definition historyR := gmapUR nat $ agree_snapshotR.
  Definition auth_historyR := authR $ historyR.
...
Section counter.
  Context (snapshotUR : ucmra) (absTUR: ucmra).
  Context {Σ} `{!heapGS Σ, !dsG snapshotUR absTUR Σ}.
  ...
  Definition hist γ_t γ_m M T (s: snapshot) : iProp :=
    ∃ (M': gmap nat (agreeR (_))),
  ...
  Lemma counterOp_spec N γ_s γ_t γ_m γ_td γ_ght template_inv op l1 l2 γ_sy t_id t0 :
          ds_inv N γ_t γ_s γ_m γ_td γ_ght template_inv -∗
            update_helping_protocol N γ_t γ_s γ_td γ_ght -∗
              thread_vars γ_t γ_ght γ_sy t_id t0 -∗
                <<< True >>> 
                     counterOp (Op_to_val op) #l1 #l2 @ ↑(cntrN N)
...
*)
  ...
  Lemma counterOp'_spec N γ_s γ_t γ_m γ_td γ_ght template_inv op l1 l2 :
          ds_inv N γ_t γ_s γ_m γ_td γ_ght template_inv -∗
              <<< ∀∀ n, Cntr γ_s n >>> 
                     counterOp' (Op_to_val op) #l1 #l2 @ ↑(cntrN N)
              <<< ∃∃ n' res, Cntr γ_s n' ∗ ⌜seq_spec op n n' res⌝, RET #res >>>.
  Proof.
